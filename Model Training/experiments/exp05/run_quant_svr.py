...
def get_objective(data_path: str, dataset_name: str, test_arch: str):
    def objective(trial):
        ...
        model, metrics = train_and_evaluate_model(model, data_path=data_path, test_arch=test_arch)

        # Log results
        logger = ExperimentLogger(experiment_id='exp05_quant_svr', dataset_name=dataset_name)
...
def main():
    ...
    for test_arch in ['a100', 'h100', 'rtx4000', 'rtx5000']:
        ...
        # Save model
        model_filename = f'exp05_quant_svr_{test_arch}_{dataset_name}.pkl'
